...
// RedisStore implements session-based seat booking backend by redis

// Key design:

// seat:{moviedID}:{seatID} -> session JSON (TTL = held, no TTL = confirmed)
...
func (s *RedisStore) ListBookings(ctx context.Context, movieID string) []Booking {
